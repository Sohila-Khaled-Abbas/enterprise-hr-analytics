...
The Microsoft SQL Server data warehouse (`EnterpriseHR_DWH`) is partitioned into three logical layers:

```
┌────────────────────────────────────────────────────────────────────────────┐
│ 1. RAW LANDING LAYER (Schema: raw)                                         │
├────────────────────────────────────────────────────────────────────────────┤
...
└─────────────────────────────────────┬──────────────────────────────────────┘
                                      ...
                                      ▼
┌────────────────────────────────────────────────────────────────────────────┐
│ 2. STAGING & TRANSFORMATION LAYER (Schema: stg)                            │
├────────────────────────────────────────────────────────────────────────────┤
...
└─────────────────────────────────────┬──────────────────────────────────────┘
                                      ...
                                      ▼
┌────────────────────────────────────────────────────────────────────────────┐
│ 3. ANALYTICAL DIMENSIONAL MART LAYER (Schema: mart)                        │
├────────────────────────────────────────────────────────────────────────────┤
...
### Complete Live Database Inventory (15 Tables, 182,108 Rows)
...
4. **`Dim_Branch`**:
   ...
   * Maps branches to geographical regions (`Greater Cairo`, `Alexandria & North`, `Delta`, `Upper Egypt`).
5. **`Dim_Course`**:
   * Connects to `Fact_TrainingCompletions` across 4 skill domains (`Tech`, `Soft Skills`, `Leadership`, `Compliance`).

---

## 5. Fact-to-Fact Cross-Filtering Best Practices in Power BI

In Power BI, multiple fact tables connected to conformed dimensions follow these architecture rules:

1. **Single-Direction Filtering**:
   * Conformed Dimensions filter Fact Tables ($1 \to *$).
   * Relationships must never be Bi-directional, avoiding ambiguity and unexpected filter propagation.
...
* **`model.tmdl`**: Registers all 9 tables, culture (`en-US`), and data access options.
* **`expressions.tmdl`**: Power Query M expressions importing the master employee dataset (`employees_data_7000.txt`) and staging tables (`stg.Exit_Attrition_Records`).
* **`tables/`**:
  * `Dim_Employee.tmdl`
  * `Dim_Department.tmdl`
  * `Dim_Branch.tmdl`
...
* **`relationships.tmdl`**: Defines all single-direction $1 \to *$ foreign key relationships linking the 5 conformed dimensions to the 4 galaxy facts.
